...
const CALENDAR_LANES = {
  ...
  frontier: { label: "Frontier v3", pnlKey: "frontierPnl", tradesKey: "frontierTradePnls" },
};
      ...
      <div className="mb-4">
        <button type="button" onClick={() => setCalendarLane("frontier")} aria-pressed={calendarLane === "frontier"}
          ...
          <div className="min-w-0">
            <div className="text-[11px] uppercase tracking-wider font-semibold text-teal-100">Frontier v3 · paper only · beats v2 holdout</div>
            ...
            <div className="text-xs text-teal-100">{tradeSummary(allTotals?.frontierTrades, allTotals?.frontierWinRate)} · score 11–15 · from 10:00 ET · flow veto · no CALL@PDL · no A+</div>
          </div>
        </button>
      </div>
            ...
            {timeframe === "Year" ? (
              ...
              <>
                ...
                <p className={`text-[11px] mt-3 ${faint}`}>
                  ...
                  Frontier v3 (live): score 11–15, from 10:00 ET, premium ≥ $0.50, no CALL@PDL/A+, plus QuantData early net-flow veto when flow opposes the trade.
                  Still short of ~75% day coverage; wider nets remain in research (`frontierV3Search.js`).
                </p>
              </>
            )}
          </div>
