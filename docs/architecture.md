...
- [x] **Real trade seeding** — `server/testAnalysis.js` seeds the verified META trade into the store on startup, idempotently (skips if already present). Now backed by real persistent storage (see below), so this only actually runs once, ever.
- [x] **Persistent trade storage (PostgreSQL)** — replaced the ephemeral JSON file. `server/db.js` manages the connection + schema (a `trades` table with a JSONB `data` column holding the full trade object, plus indexed `symbol`/`entry_date`/`exit_date` columns for future querying). `server/store.js`'s `readTrades`/`appendTrade` are now async and hit Postgres. Requires a `DATABASE_URL` variable on GHOSTFLOW's own Railway service — being in the same Railway *project* as the Postgres service does not auto-inject it; it must be added as an explicit variable reference. Fixed a real bug during this migration: a DB connection failure during the startup seed step was crashing the entire server process rather than just skipping that step — now caught and logged, server stays up regardless.
...
- [x] **AI tool use — models can fetch their own data** — `server/tools.js` gives each analyst a real `fetch_market_data` tool (any symbol, any date range, daily/intraday/options-flow) via `server/aiProviders.js`'s tool-calling loop (Claude/GPT/Grok, each provider's native format). No artificial limit on what they fetch — only a generous 8-round circuit breaker against a runaway loop. Grok can now genuinely fetch QQQ itself to build a relative-strength comparison instead of us pre-deciding its data. NOTE: fetched data feeds the model's *reasoning and citations* (flags/text); it is not yet captured back into the interactive chart as a plotted overlay line — that's a separate, not-yet-built piece.
- [ ] **Grok relative-strength benchmark overlay (chart)** — Grok can now fetch QQQ data for its own reasoning, but the interactive chart doesn't visualize it. Would need the tool-fetched series captured and returned alongside the trade so it can be plotted.
